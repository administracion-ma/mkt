...
export type KeyInsight = { icon: string; text: string; tone: "good" | "warn" | "bad" | "neutral" };
...
export function buildKeyInsights(args: {
  ...
  if (hookMedianSkip != null) {
    if (hookMedianSkip > 50) {
      ...
      insights.push({
        icon: "🚨",
        tone: "bad",
        text: `Skip rate crítico (${hookMedianSkip.toFixed(0)}%): la mayoría abandona tus reels sin verlos${worst?.caption ? ` — el peor caso es "${worst.caption.slice(0, 50)}"` : ""}. Revisá el primer segundo.`,
      });
  ...
  if (pillarStats.length >= 2) {
    ...
    if (top.erMediana != null) {
      insights.push({
        icon: "🏆",
        tone: "good",
        text: `"${top.label}" es tu pilar más fuerte (ER mediana ${(top.erMediana * 100).toFixed(1)}%, ${top.posts} posts) — subí la frecuencia de ese tipo de contenido.`,
      });
    }
  ...
  if (hashtagStats.length > 0) {
    ...
    insights.push({
      icon: "#️⃣",
      tone: "neutral",
      text: `${top.tag} está en tus posts de mejor alcance (mediana ${fmt(top.alcanceMediano)} en ${top.posts} posts) — seguí usándolo.`,
    });
  }
